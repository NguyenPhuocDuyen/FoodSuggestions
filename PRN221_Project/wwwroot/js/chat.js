$(document).ready(function () {
    ...
    connection.on("addUser", function (user) {
        viewModel.userAdded(new ChatUser(user.username, user.firstName, user.avatar, user.currentRoom));
    });
    ...
    function AppViewModel() {
        ...
        self.deleteRoom = function () {
            fetch('/api/Room/' + self.joinedRoomId(), {
                method: 'DELETE',
                headers: { 'Content-Type': 'application/json' },
        ...
        self.messageHistory = function () {
            fetch('/api/Message/Room/' + viewModel.joinedRoom())
                .then(response => response.json())
                .then(data => {
